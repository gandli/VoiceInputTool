...
DEFAULT_CONFIG = {
    ...
    "logging": {
        "level": "INFO",
        "file": "voice_input_client_linux.log",
        "max_size_mb": 10,
        "backup_count": 5,
    },
    "security": {
...
class VoiceInputClient:
    ...
    def _setup_logging(self):
        ...
        log_file = self.config.get("logging", {}).get("file", "voice_input_client_linux.log")
        
        self.logger = logging.getLogger("VoiceInputToolLinux")
        ...
        self.logger.info(f"Linux client initialized (ID: {platform.node()})")
    ...
    def find_voice_input_device(self) -> Optional[str]:
        ...
        try:
            ...
            # Common Linux USB serial device patterns
            device_patterns = [
            ...
            for port in ports:
                ...
                if any(pattern in device for pattern in device_patterns):
                    self.logger.info(f"Found potential device: {device} - {port.description}")
                    return device
                
                # Also check for generic USB serial
                if 'usb' in description.lower() or 'cp210' in description.lower():
    ...
    def connect_to_device(self) -> bool:
        ...
        try:
            ...
            if "Permission denied" in error_msg:
                ...
                self.logger.error("Device is busy. Close other programs using this port.")
    ...
    def start(self):
        ...
        self.logger.info("Starting Voice Input Tool - Linux Client")
...
def main():
    """Main entry point."""
    print(f"Voice Input Tool - Linux Client")
